...
def _get_gitlab_url_from_git() -> str:
...
def _get_gitlab_client() -> Gitlab:
    """Initialize and return a GitLab client."""
    gitlab_url = os.getenv("GITLAB_URL")
    if not gitlab_url:
        gitlab_url = _get_gitlab_url_from_git()
    ...
    return Gitlab(gitlab_url, private_token=gitlab_token, timeout=10.0)


def _get_project_name_from_mr(gl: Gitlab, mr) -> str:
...
def get_user_open_mrs(asc: bool = False) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for mr in mrs:
            ...
            project_name = _get_project_name_from_mr(gl, mr)
